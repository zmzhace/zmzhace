...
目前我正全力投入于 **[Open-Cowork](https://github.com/zmzhace/open-cowork)** 的核心攻坚。这不仅仅是一个跨平台协同工作空间，更是我向 **AGI (通用人工智能)** 迈进的实验场。近期的核心研发方向包括：
- **纯视觉驱动智能体 (Pure-Vision Agent)**: 摒弃传统的底层 API 依赖，全面转向基于实时屏幕截图的 VLM（视觉语言模型）理解。
- **基于 ReAct 范式的 Computer Use**: 结合大模型的推理能力，让 Agent 能够像人类一样，通过“观察屏幕 -> 思考 (CoT) -> 执行鼠标/键盘动作”的 ReAct 认知闭环，实现对任意 GUI 软件的全自动接管与操作。
- **构建通用的跨平台操作基石**: 致力于打造一套不受限于操作系统的泛用型系统控制接口，为下一代端侧智能体 (On-Device AI) 提供极其透明、可追溯的执行环境。

如果您对 `Computer Use` 商业落地、纯视觉多模态智能、或是 AGI 基础设施感兴趣，非常欢迎与我交流！
